...
本章参考自 [NearXDU的博客](https://blog.csdn.net/zhangxiao93/article/details/53677764)

确认vim编辑器是否支持**clipboard**特性
...
如果不支持怎么办？安装新的vim吧。可以参考这边文章 [从源码安装vim](https://github.com/ycm-core/YouCompleteMe/wiki/Building-Vim-from-source)

### vim的寄存器

本章参考自 [rainysia的博客](https://blog.csdn.net/rainysia/article/details/7721691)

vim会有一些存储空间来暂存内容，vim中将这些存储空间叫**寄存器**。此寄存器非我们硬件所说的寄存器，可以理解为是一种cache。

例如我们在vim中`y`操作复制的内容，实际就是暂存到寄存器中。
...
https://zhuanlan.zhihu.com/p/95596162
